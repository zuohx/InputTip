...
fn_input_mode(*) {
    ...
    inputModeGui(info) {
        ...
        g.AddText("xs", "4. 是否将输入法状态导出:")
        g.AddDropDownList("yp Choose" exportState + 1, ["【否】", "【是】"]).OnEvent("Change", e_export_state)
        e_export_state(item, *) {
            value := item.value - 1
            global exportState := value
            writeIni("exportState", value)
            if (exportState == 0 && FileExist(exportStateFile)) {
                FileDelete(exportStateFile)
            }
        }
        g.AddLink("xs cGray", '启用之后，它会实时写入到 %TEMP%\abgox.InputTip.State 文件中`n这用于其他外部脚本或程序去读取 InputTip 识别的输入法状态，如 <a href="https://inputtip.abgox.com/faq/inputtip-for-vscode">InputTip for VSCode</a>')

        g.AddText("xs", "5. 输入法状态的获取超时:")
        ...
        g.AddText("xs cGray", "单位：毫秒，默认为 500 毫秒，非特殊情况不应该随意修改它`n如果超过了这个时间，则判定获取失败，直接判断为英文状态")
        tab.UseTab(2)
        ...
        gc.input_mode_LV := _ := g.AddListView("xs -LV0x10 -Multi r9 NoSortHdr Grid w" w, ["匹配的顺序", "状态码规则", "切换码规则", "输入法状态"])
        fn_reloading_LV(_)
